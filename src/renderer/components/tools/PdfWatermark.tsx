import { useState, useCallback } from 'react'
import { Play, Type, ImageIcon } from 'lucide-react'
import { FileDropZone } from '../shared/FileDropZone'
import { ProcessingStatus } from '../shared/ProcessingStatus'
import { useTask, getDefaultOutputPath } from '@/hooks/useTask'
import { PDF_FILTERS, IMAGE_FILTERS } from '@shared/constants'

type WatermarkType = 'text' | 'image'

export function PdfWatermark(): JSX.Element {
  const [file, setFile] = useState<File | null>(null)
  const [outputPath, setOutputPath] = useState<string>('')
  const [watermarkType, setWatermarkType] = useState<WatermarkType>('text')
  const [text, setText] = useState<string>('CONFIDENTIAL')
  const [imagePath, setImagePath] = useState<string>('')
  const [opacity, setOpacity] = useState<number>(30)

  const { isProcessing, progress, error, result, execute, reset } = useTask({
    taskType: 'pdf:watermark'
  })

  const handleFilesSelected = useCallback((files: File[]) => {
    if (files.length > 0) {
      setFile(files[0])
      const firstFilePath = window.api.file.getFilePath(files[0])
      setOutputPath(getDefaultOutputPath(firstFilePath, '_watermarked'))
    }
  }, [])

  const handleSelectOutput = async () => {
    const path = await window.api.file.saveFile({
      defaultPath: outputPath,
      filters: PDF_FILTERS
    })
    if (path) setOutputPath(path)
  }

  const handleSelectImage = async () => {
    const files = await window.api.file.openFile({
      multiple: false,
      filters: IMAGE_FILTERS
    })
    if (files && files.length > 0) {
      setImagePath(files[0])
    }
  }

  const handleProcess = async () => {
    if (!file || !outputPath) return
    const filePath = window.api.file.getFilePath(file)

    const watermark = watermarkType === 'text'
      ? { text, opacity: opacity / 100 }
      : { image: imagePath, opacity: opacity / 100 }

    await execute(() => window.api.pdf.addWatermark(filePath, outputPath, watermark))
  }

  const handleReset = () => {
    reset()
    setFile(null)
    setOutputPath('')
    setWatermarkType('text')
    setText('CONFIDENTIAL')
    setImagePath('')
    setOpacity(30)
  }

  const handleOpenResult = () => {
    if (result) {
      const dir = result.substring(0, Math.max(result.lastIndexOf('/'), result.lastIndexOf('\\')))
      window.api.file.openPath(dir)
    }
  }

  const canProcess = file && outputPath && (
    (watermarkType === 'text' && text) ||
    (watermarkType === 'image' && imagePath)
  )

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-100 mb-2">Add Watermark</h1>
        <p className="text-gray-400">Add text or image watermark to PDF pages</p>
      </div>

      {!result && (
        <>
          <FileDropZone
            accept={{ 'application/pdf': ['.pdf'] }}
            onFilesSelected={handleFilesSelected}
            multiple={false}
            disabled={isProcessing}
            hideFileList={true}
          />

          {file && (
            <>
              <div className="card">
                <h3 className="font-medium text-gray-200 mb-4">Selected File</h3>
                <div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                  <span className="flex-1 text-gray-200 truncate">{file.name}</span>
                  <span className="text-gray-500 text-sm">
                    {(file.size / 1024 / 1024).toFixed(2)} MB
                  </span>
                </div>
              </div>

              <div className="card space-y-4">
                <h3 className="font-medium text-gray-200">Watermark Options</h3>

                <div>
                  <label className="text-sm text-gray-400 block mb-3">Watermark Type</label>
                  <div className="flex gap-3">
                    <button
                      onClick={() => setWatermarkType('text')}
                      disabled={isProcessing}
                      className={`flex-1 p-3 rounded-lg border flex items-center justify-center gap-2 transition-colors ${
                        watermarkType === 'text'
                          ? 'border-primary-500 bg-primary-500/10 text-primary-400'
                          : 'border-white/10 hover:border-white/20 text-gray-400'
                      }`}
                    >
                      <Type className="w-5 h-5" />
                      <span>Text Watermark</span>
                    </button>
                    <button
                      onClick={() => setWatermarkType('image')}
                      disabled={isProcessing}
                      className={`flex-1 p-3 rounded-lg border flex items-center justify-center gap-2 transition-colors ${
                        watermarkType === 'image'
                          ? 'border-primary-500 bg-primary-500/10 text-primary-400'
                          : 'border-white/10 hover:border-white/20 text-gray-400'
                      }`}
                    >
                      <ImageIcon className="w-5 h-5" />
                      <span>Image Watermark</span>
                    </button>
                  </div>
                </div>

                {watermarkType === 'text' ? (
                  <div>
                    <label className="text-sm text-gray-400 block mb-2">Watermark Text</label>
                    <input
                      type="text"
                      value={text}
                      onChange={(e) => setText(e.target.value)}
                      placeholder="Enter watermark text..."
                      className="input-field w-full"
                      disabled={isProcessing}
                    />
                  </div>
                ) : (
                  <div>
                    <label className="text-sm text-gray-400 block mb-2">Watermark Image</label>
                    <div className="flex gap-3">
                      <input
                        type="text"
                        value={imagePath}
                        readOnly
                        placeholder="Select an image..."
                        className="input-field flex-1"
                      />
                      <button
                        onClick={handleSelectImage}
                        className="btn-secondary"
                        disabled={isProcessing}
                      >
                        Browse
                      </button>
                    </div>
                  </div>
                )}

                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <label className="text-gray-400">Opacity</label>
                    <span className="text-gray-300">{opacity}%</span>
                  </div>
                  <input
                    type="range"
                    min={5}
                    max={100}
                    value={opacity}
                    onChange={(e) => setOpacity(parseInt(e.target.value))}
                    className="w-full"
                    disabled={isProcessing}
                  />
                </div>
              </div>

              <div className="card space-y-4">
                <h3 className="font-medium text-gray-200">Output</h3>
                <div className="flex gap-3">
                  <input
                    type="text"
                    value={outputPath}
                    onChange={(e) => setOutputPath(e.target.value)}
                    placeholder="Select output path..."
                    className="input-field flex-1"
                    disabled={isProcessing}
                  />
                  <button
                    onClick={handleSelectOutput}
                    className="btn-secondary"
                    disabled={isProcessing}
                  >
                    Browse
                  </button>
                </div>

                <button
                  onClick={handleProcess}
                  disabled={isProcessing || !canProcess}
                  className="btn-primary w-full flex items-center justify-center gap-2"
                >
                  <Play className="w-4 h-4" />
                  Add Watermark
                </button>
              </div>
            </>
          )}
        </>
      )}

      <ProcessingStatus
        isProcessing={isProcessing}
        progress={progress}
        error={error}
        result={result}
        onOpenResult={handleOpenResult}
        onReset={handleReset}
      />
    </div>
  )
}

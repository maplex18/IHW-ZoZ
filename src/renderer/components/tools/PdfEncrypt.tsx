...
export function PdfEncrypt(): JSX.Element {
  ...
  const handleFilesSelected = useCallback((files: File[]) => {
    if (files.length > 0) {
      ...
      setOutputPath(getDefaultOutputPath(firstFilePath, '_encrypted'))
    }
  }, [])
  ...
  const handleProcess = async () => {
    ...
    await execute(() => window.api.pdf.encrypt(filePath, outputPath, password))
  }
  ...
  const canProcess = file && outputPath && password && passwordsMatch

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-100 mb-2">Encrypt PDF</h1>
        <p className="text-gray-400">Password protect your PDF document</p>
      </div>
          ...
          {file && (
            <>
              ...
              <div className="card space-y-4">
                ...
                <div>
                  <label className="text-sm text-gray-400 block mb-2">Enter Password</label>
                ...
                <div className="p-3 bg-yellow-500/10 rounded-lg">
                  <p className="text-yellow-400 text-sm">
                    Make sure to remember this password. It cannot be recovered if lost.
                  </p>
                </div>
              </div>
